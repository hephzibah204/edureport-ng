...
async function handleCreateStudent(db, request, session, origin) {
  const body = await request.json();
  const { name, cls, className, gender, admNo, admissionNo, photoUrl, dob, club } = body;
  const id = generateId();
...
async function handleImportExam(db, request, session, env, origin) {
  const body = await request.json();
  const { subject, classLevel, term, session: schoolSession, examType, duration, textContent } = body;
...
// ../.wrangler/tmp/pages-X9PHJF/functionsRoutes-0.5379238676447805.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-zdNP6B/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-zdNP6B/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5079872866638288.mjs.map
