...
// .wrangler/tmp/pages-jHRu8k/functionsWorker-0.05202549198050421.mjs
...
async function onRequest(context) {
  ...
  if (url.pathname === "/api/admin/bucket" && method === "GET") {
    if (!env.BUCKET) return errorResponse("No BUCKET binding", 500);
    const listed = await env.BUCKET.list();
    return jsonResponse(listed.objects.map((o) => o.key));
  }
  try {
    if (!env.DB) {
...
async function generateAIResponse(env, prompt, maxTokens = 4096) {
  if (env.AI) {
    try {
...
__name2(generateAIResponse, "generateAIResponse");
...
async function handleGenerateExam(db, request, session, env, origin) {
  ...
  if (sourceMode === "curriculum") {
    const termLabel2 = term || "2nd Term";
    contextText = `Generate questions strictly based on the ${curriculum || "NERDC Scheme"} of work for ${subject} in ${classLevel} for ${termLabel2}. Check your knowledge base for the exact curriculum standards of the selected scheme in Nigeria.`;
...
async function handleUpdateExam(db, request, examId, session, env, origin) {
  const { subject, classLevel, curriculum, topic, questions, term, session: schoolSession, examType, questionType, duration, isShared } = await request.json();
  ...
  await db.update(exams).set({
    ...
    topic: updatedTopic !== void 0 ? updatedTopic : existing.topic,
...
Keep it to 2-3 sentences max. Be encouraging but honest based on the scores.`;
  try {
    const aiResponse = await generateAIResponse(env, prompt, 1e3);
...
async function handleBulkImportDocumentExam(db, request, session, env, origin) {
  try {
    const body = await request.json();
...
__name(handleBulkImportDocumentExam, "handleBulkImportDocumentExam");
__name2(handleBulkImportDocumentExam, "handleBulkImportDocumentExam");
...
// .wrangler/tmp/bundle-CHTKo9/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-CHTKo9/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.05202549198050421.js.map
