...
export default function AdminReports() {
  ...
  // Use SWR with dynamic query params for filtering
  const { data: scoresData, isLoading: scoresLoading, mutate: mutateScores } = useSWR(
    `/api/scores?session=${encodeURIComponent(selectedSession)}&term=${encodeURIComponent(selectedTerm)}`, 
    fetcher
  );
  ...
  // Set default session and term from school settings
  useEffect(() => {
    if (school) {
      if (school.session) setSelectedSession(school.session);
      if (school.term) setSelectedTerm(school.term);
    }
  }, [school]);
          ...
          <div className="space-y-6">
            ...
            <div className="glass p-8 rounded-[2rem] shadow-elite bg-gradient-to-r from-indigo-50/50 to-emerald-50/30 border border-indigo-100/50 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex-1">
                ...
                <div className="flex flex-wrap items-center gap-4 mt-3 no-print">
                  <div className="flex flex-col">
                    <label className="text-[10px] font-black uppercase text-[#464555]/60 mb-1">Academic Session</label>
                    <select 
                      value={selectedSession}
                      onChange={(e) => setSelectedSession(e.target.value)}
                      className="px-3 py-1.5 bg-white border border-indigo-100 rounded-lg text-xs font-bold text-[#0b1c30] outline-none shadow-sm"
                    >
                  ...
                  <div className="flex flex-col">
                    <label className="text-[10px] font-black uppercase text-[#464555]/60 mb-1">Term</label>
                    <select 
                      value={selectedTerm}
                ...
                <p className="text-sm font-semibold text-[#464555]/60 mt-3 print:block hidden">{selectedSession} | {selectedTerm} | Total Students: {rawBroadsheetData.length}</p>
              </div>
