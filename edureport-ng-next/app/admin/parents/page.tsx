  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(error.error?.message || 'Failed to invite parent');
      }
    } catch (error) {
      ...
      <div className="space-y-6">
        ...
        <section className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass p-6 rounded-3xl shadow-elite">
          ...
          <div className="flex items-center gap-3">
            <button className="p-3 bg-[#f8f9ff] border border-[#0b1c30]/5 rounded-xl text-[#464555] hover:text-indigo-600 transition-colors">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="text-[10px] font-extrabold text-[#464555]/40 uppercase tracking-widest border-b border-[#0b1c30]/5 bg-[#f8f9ff]/50">
                  <th className="px-8 py-6">Parent / Guardian</th>
                  <th className="px-8 py-6">Contact Info</th>
                  <th className="px-8 py-6">Linked Students</th>
                  ...
                  <tr>
                    <td colSpan={5} className="px-8 py-20 text-center">
                      <div className="flex flex-col items-center gap-3">
                        ...
                        <p className="text-lg font-bold text-[#0b1c30]">No parents found</p>
                        <p className="text-sm text-[#464555]/60">Try adjusting your search or invite a new parent</p>
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr key={parent.id} className="group hover:bg-indigo-50/30 transition-colors">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <div className="font-bold text-[#0b1c30] group-hover:text-indigo-600 transition-colors">{parent.displayName}</div>
                          <div className="flex items-center gap-1.5 text-[10px] font-bold text-[#464555]/50 mt-0.5">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1">
                        ...
                        {parent.phone && (
                          <div className="flex items-center gap-2 text-xs font-medium text-[#464555]/70">
                            <Phone className="w-3 h-3 text-[#464555]/30" />
                            {parent.phone}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col gap-1.5">
                        {parent.linkedStudents && parent.linkedStudents.length > 0 ? (
                          parent.linkedStudents.map((student: any) => (
                            <div key={student.id} className="flex items-center gap-2 text-sm font-bold text-[#0b1c30]">
                              <GraduationCap className="w-4 h-4 text-indigo-600" />
                              {student.name} <span className="text-[10px] font-medium text-[#464555]/50 px-2 py-0.5 rounded-md bg-[#0b1c30]/5">{student.cls}</span>
                            </div>
                          ))
                        ) : (
                          <span className="text-xs font-medium text-[#464555]/40 italic">No students linked</span>
                        )}
                      </div>
                    </td>
                    <td className="px-8 py-6">
                    ...
                    <td className="px-8 py-6 text-right">
                      <button className="w-10 h-10 rounded-xl flex items-center justify-center text-[#464555]/30 hover:text-indigo-600 hover:bg-white transition-all">
